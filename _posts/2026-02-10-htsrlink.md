---
layout: post
title: A Trivial Toy Model to bridge HTSR with Alpha-REQ
date: 2026-02-10
...
1. First, we will quickly relate the condition number of a matrix to the decay of the singular value spectrum. 
...
It is quite simple to see that a steeper decay in the singular value spectrum of a matrix should correspond to a higher condition number and vice versa, as flatter spectra imply that the minimum singular value is still reasonable close to the maximum singular value. Let us prove this formally.
...
Since $u_{\min} = m \ u_{\max}$ and substituting in the expression for $\kappa(A)$ gives us

$$
\kappa(A) = \frac{\lambda_{\max}}{\lambda_{\min}} = \frac{c_0 (u_{\min})^{-\alpha}}{c_0 (m u_{\max})^{-\alpha}} = m^{\alpha}
$$
...
Corresondingly, $\sigma_{\max}(AB) \leq \sigma_{\max}(A) \ \sigma_{\max}(B)$ and $\sigma_{\min}(AB) \geq \sigma_{\min}(A) \ \sigma_{\min}(B)$ give the above result for the condition number of a matrix multiplication. Then, we can see that
...
$$
c_{Y} \cdot m^{-\alpha_Y} \leq (C_{\text{data}} \cdot c_{W}) \cdot m^{-\alpha_W} = m^{-\alpha_Y} \leq \frac{C_{\text{data}} \cdot c_{W}}{c_{Y}} \cdot m^{-\alpha_W}
$$
