...
Since ReLU-like non-linearity zero-out negative inputs, it is possible that $\text{rank}(\widetilde{Y}^\top \widetilde{Y}) < m$. Using the bound on condition number of the product of two matrices, we now write
